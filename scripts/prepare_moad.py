...
def process_sdf(sdf_paths, proteins_dir, progress=True):
    ...
    sdf_path_list= tqdm(sdf_paths, total=len(sdf_paths)) if progress else sdf_paths
    ...
    for sdf_path in sdf_path_list:
        ...
        pdb_path = os.path.join(proteins_dir, f'{pdb_code}.pdb')
...
def run(
    ...
    sdf_paths = glob.glob(test_path +'/*.sdf')
    molecules, pockets, out_table = process_sdf(sdf_paths, test_path, progress)
    ...
    with Chem.SDWriter(open(out_mol_path, 'w')) as writer:
        for i, mol in enumerate(molecules):
            for atom in mol.GetAtoms():
                ...
                if num_radicals != 0:
                    raise ValueError("There are radicals!!!!")
            try:
                writer.write(mol)
            except:
                print('dummy')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_path', action='store', type=str, required=True)
