...
def make_data_placeholder(n_graphs, device=None, mean_size=None):
    # n_nodes_list = np.random.randint(15, 50, n_graphs)
    if mean_size is None:  # use statistics from GEOM-Drug dataset
        n_nodes_list = np.random.normal(24.923464980477522, 5.516291901819105, size=n_graphs)
    else:
        # n_nodes_list = np.array([max_size] * n_graphs)
        n_nodes_list = np.random.normal(mean_size, 1, size=n_graphs)
