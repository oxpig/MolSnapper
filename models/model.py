...
class MolDiff(Module):
    def __init__(self,
        ...
        self.donor_type = torch.tensor([1])
        self.acceptor_type = torch.tensor([2])
    ...
    def sample(self, n_graphs, batch_node, halfedge_index, batch_halfedge, bond_predictor=None, guidance=None):
        ...
        for i, step in tqdm(enumerate(range(self.num_timesteps)[::-1]), total=self.num_timesteps):
            ...
            if self.categorical_space == 'discrete':
                # node types
                log_node_recon = F.log_softmax(pred_node, dim=-1)
                log_node_type = self.node_transition.q_v_posterior(log_node_recon, log_node_type, time_step, batch_node,
                                                                   v0_prob=True)
                ...
                log_halfedge_type = self.edge_transition.q_v_posterior(log_edge_recon, log_halfedge_type, time_step,
                                                                       batch_halfedge, v0_prob=True)
                    ...
                    with torch.enable_grad():
                        ...
                        if gui_type == 'entropy':
                            ...
                            prob_halfedge_type = log_halfedge_type.exp()[:,
                                                 :-1]  # the last one is masked bond (not used in predictor)
                            entropy = F.cross_entropy(pred_bondpredictor, prob_halfedge_type, reduction='none')
                            entropy = entropy.log().sum()
                            delta = - torch.autograd.grad(entropy, pos_in)[0] * gui_scale
                        elif gui_type == 'crossent_bond':
                            prob_halfedge_type = log_halfedge_type.exp()[:,
                                                 1:-1]  # the last one is masked bond. first one is no bond
    ...
    @torch.no_grad()
    def sample_in_pocket(self, n_graphs, batch_node, halfedge_index, batch_halfedge, ref_coords=None, ref_mask=None,
                         ref_type=None, log_dir=None, bond_predictor=None, guidance=None, condition_clash=False,
                         pocket_x=None, clash_rate=0.0):
        ...
        # Get the indices where pharmacophore_mask equals 1
        indices = torch.nonzero(ref_mask == 1).squeeze()

        # Generate random indices to sample from pharmacophore_coords
        random_indices = torch.randint(0, indices.size(0), (ref_coords.size(0),))

        # Fill pharmacophore_init with random elements from pharmacophore_coords
        ref_init = ref_coords[indices[random_indices]]
        ref_init[ref_mask == 1] = ref_coords[ref_mask == 1]

        # pos_init = pos_init + ref_init
        halfedge_init = self.edge_transition.sample_init(n_halfedges_all)
        if self.categorical_space == 'discrete':
            _, h_node_init, log_node_type = node_init
            _, h_halfedge_init, log_halfedge_type = halfedge_init
        ...
        pos_traj[0] = pos_init
        halfedge_traj[0] = h_halfedge_init
        pocket_schedule = self.pos_transition.alphas_bar.sqrt()
        ...
        for i, step in tqdm(enumerate(range(self.num_timesteps)[::-1]), total=self.num_timesteps):
            ...
            # # 2 get the t - 1 state
            # pos
            pos_prev, gradients = self.pos_transition.get_prev_from_recon_pocket(
                x_t=pos_pert, x_recon=pred_pos, t=time_step, batch=batch_node, n_samples=n_graphs, ref=ref_coords,
                ref_mask=ref_mask,
                condition_clash=condition_clash, pocket_x=pocket_x, clash_rate=clash_rate, device=device)
            ...
            if self.categorical_space == 'discrete':
                ...
                log_node_type = self.node_transition.q_v_posterior_with_ref(log_node_recon, log_node_type, time_step,
                                                                            batch_node,
                                                                            v0_prob=True, ref_type=ref_type,
                                                                            ref_mask=ref_mask)
                ...
                if (gui_scale > 0):
                    with torch.enable_grad():
                        ...
                        if gui_type == 'entropy':
                            ...
                            prob_halfedge_type = log_halfedge_type.exp()[:,
                                                 1:-1]  # the last one is masked bond. first one is no bond
                            entropy = F.cross_entropy(pred_bondpredictor[:, 1:], prob_halfedge_type,
                                                      reduction='none')
                            entropy = entropy.log().sum()
                            delta = - torch.autograd.grad(entropy, pos_in)[0] * gui_scale
                        else:
